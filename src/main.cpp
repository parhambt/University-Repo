...
const string EASY_ = "Easy";
const string MEDIUM_ = "Medium";
const string HARD_ = "Hard";
...
public:
    ...
    static map<string, vector<int>> get_statistics_exam(Exam *exam)
    {
...
public:
    ...
    static vector<pair<string, double>> subject_ratio_correct_answer()
    {
        ...
        for (auto subject : all_subject)
        {
            ...
            {
                if (question->choosen_option == stoi(question->correct_answer))
                    count_true++;
                all_seen_questions += question->num_seen_question;
            }
            if (all_seen_questions == 0)
                answer.push_back(make_pair(subject, 0));
            else
                answer.push_back({subject, (double)count_true / all_seen_questions});
        }
        return answer;
    }
    ...
    {
        if (specific_part_question == "question_text")
            return this->question_text;
        else if (specific_part_question == "option1")
            return this->option1;
        else if (specific_part_question == "option2")
            return this->option2;
        else if (specific_part_question == "option3")
            return this->option3;
        else if (specific_part_question == "option4")
            return this->option4;
        else
            return "NONE";
    }
    ...
    {
        if (this->correct_answer == choosen_answer)
            return true;
        else
            return false;
    }
    ...
    {
        if (choosen_answer == "1" || choosen_answer == "2" || choosen_answer == "3" || choosen_answer == "4" || choosen_answer.empty())
            return true;
        else
            return false;
    }
    ...
    static map<string, vector<int>> add_subject(string exam_name)
    {
        map<string, vector<int>> initial_exam_statistics;
    ...
    static void choose_answer(const pair<string, map<Questions *, string>> &questions_with_answer)
    {

        string exam_name = questions_with_answer.first;
        auto exam = Exam::get_exam(exam_name);
        map<string, vector<int>> temp_exam_statistics = Questions::add_subject(exam_name);

        for (const auto &[question, answer] : questions_with_answer.second)
        {
            string subject = question->get_subject();
            if (answer.empty())
                question->choosen_option = 0;
            else
                question->choosen_option = stoi(answer);
    ...
    static bool sort_priority(Questions *a, Questions *b)
    {
        ...
        {
            if (a->question_text < b->question_text)
                return true;
            else
                return false;
        }
    ...
    static vector<string> find_all_exam_subject(string exam_name)
    {
        auto exam = Exam::get_exam(exam_name);
...
public:
    static void input_handelling()
    {
        ...
        while (getline(cin, line))
        {
            ...
            else if (input == ATTEND)
            {
                ...
                if (exam == nullptr)
                    cout << "Could not find test: \'" << test_name << "\'\n";
                else
                {
    ...
    static string capitalize_first_letter(string s)
    {
        if (!s.empty())
        {
            s[0] = std::toupper(s[0]);
    ...
    static void print_report(int stage, string test_name)
    {
        if (stage == 0)
            ...
            cout << "Results for " << test_name << ":\n";
        map<string, vector<int>> statistics;
        if (stage == 0)
            statistics = Report::report_statistics_categorical(Questions::get_all_questions_vector());
        else
        {
    ...
    static void print_test(string test_name)
    {
        ...
        vector<int> all_statistics = {0, 0, 0};
        for (const auto &[subject, statistic] : statistics)
        {
            string score = three_figure(Report::calculate_score(statistic));
            cout << subject << ": " << statistic[0] << " corrects, " << statistic[1] << " incorrects and " << statistic[2] << " blanks. Score: " << score << ".\n";
        ...
        string total_score = three_figure(Report::calculate_score(all_statistics));
        cout << "Total score: " << total_score << ".\n";
    }
    ...
    {
        pair<string, map<Questions *, string>> questions_with_answer;
    ...
    static void option_printer(int choosen_answer, const vector<Questions *>::iterator it, int count, int count_prev)
    {
        ...
        if (choosen_answer == 1 && count_prev > 0)
            cout << TAB << "1. " << (*it)->get_question_detail(OPTION1) << FLASH << endl;
        else
            cout << TAB << "1. " << (*it)->get_question_detail(OPTION1) << endl;

        if (choosen_answer == 2 && count_prev > 0)
            cout << TAB << "2. " << (*it)->get_question_detail(OPTION2) << FLASH << endl;
        else
            cout << TAB << "2. " << (*it)->get_question_detail(OPTION2) << endl;

        if (choosen_answer == 3 && count_prev > 0)
            cout << TAB << "3. " << (*it)->get_question_detail(OPTION3) << FLASH << endl;
        else
            cout << TAB << "3. " << (*it)->get_question_detail(OPTION3) << endl;

        if (choosen_answer == 4 && count_prev > 0)
            cout << TAB << "4. " << (*it)->get_question_detail(OPTION4) << FLASH << endl;
        else
            cout << TAB << "4. " << (*it)->get_question_detail(OPTION4) << endl;
    }
    static bool enter_option(vector<Questions *>::iterator &it, bool &is_valid_option, vector<Questions *> &questions, pair<string, map<Questions *, string>> &questions_with_answer)
    {
        while (is_valid_option != true)
        {
            ...
            if (is_valid_option)
                questions_with_answer.second.insert_or_assign((*it), answer);
            if (answer == "previous" && questions.begin() != it)
                return false;
            else if (answer == "previous" && questions.begin() == it)
